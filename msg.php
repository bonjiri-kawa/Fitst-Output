...
//DBから掲示板とメッセージデータを取得
$bordData = getBord($m_id);
if(empty($bordData)){
  error_log('ボードデータがありません');
  ("Location:mypage.php");
  exit();
}
if(in_array($_SESSION['user_id'], $bordData, true) === false){
  header("Location:mypage.php");
  exit();
}
$msgData = getMsg($m_id);
$productInfo = getProductOne($bordData['product_id']);
...
$dealUserIds[] = $bordData['sale_user'];
//var_dump('売り手',$dealUserIds);
$dealUserIds[] = $bordData['buy_user'];
//var_dump('買い手',$dealUserIds);
...
//DBから取引相手のユーザーデータ情報を取得
$partnerUserInfo = getUser($partnerUserId);
debug('取得したデータ：'.print_r($partnerUserInfo, true));
...
var_dump($myUserInfo);
debug('取得したデータ:'.print_r($myUserInfo, true));
//自分のユーザー情報が取れたかチェック
if(empty($myUserInfo)){
      ...
      <div class="msg-info">
        ...
        <div class="product-info">
          ...
          <div class="right">
            ...
            取引開始日:<?php echo date('Y/m/d', strtotime(sanitize($bordData['create_date']))); ?>
          </div>
        </div>
      </div>
